...
func Fill_the_sheet(Category string, Type string, Tag string, Price string) {
	...
	data1D := []string{date, Category, Type, Tag, Price}
	...
	Tab := fmt.Sprintf("%02d!A2:E2", now.Month())
	_, err = srv.Spreadsheets.Values.Append(spreadsheetId, Tab, &vr).ValueInputOption("USER_ENTERED").Do()
}
